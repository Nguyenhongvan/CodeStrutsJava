	...
	public ActionErrors validate(ActionMapping mapping,
		 ...
		 ActionErrors errors = new ActionErrors();
	        if ("them".equalsIgnoreCase(getSubmit())) {
		        ...
		        if(errors.isEmpty()){
		            if(!formFile.getContentType().equalsIgnoreCase("image/jpeg") && !formFile.getContentType().equalsIgnoreCase("image/jpg")
		            && !formFile.getContentType().equalsIgnoreCase("image/pjpeg")){
		            	 errors.add("uploadedFile", new ActionMessage("errors.file.type", formFile.getFileName(),formFile.getContentType()));
			            	System.out.println("loi images abc"+formFile.getFileName()+" "+formFile.getContentType());
		            }
		               
		            if(formFile.getFileSize() > 500000){
		            	  errors.add("uploadedFile", new ActionMessage("errors.file.size",formFile.getFileName(),formFile.getFileSize()));
